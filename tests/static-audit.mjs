...
const [index, privacy, script, styles] = await Promise.all([
  read('index.html'),
  read('privacy.html'),
  read('script.js'),
  read('styles.css'),
...
test('the skip link is the first interactive control on both public pages', () => {
...
test('all repository-local asset references resolve', async () => {
  const refs = new Set();
  for (const source of [index, privacy]) {
