  ...
  const fragmentSource = `
    ...
    void main() {
      ...
      vec2 bodyShift = vec2(centered.x * u_rect.z, -centered.y * u_rect.w)
        * mix(.0015, .0042, u_open);
      float edgeStrength = mix(5.0, 13.0, u_open);
      ...
      float blurRadius = mix(3.6, 8.2, u_open) + edgeLens * .9;
      ...
      float topLight = (1.0 - smoothstep(.0, .085, 1.0 - v_uv.y)) * .19;
      float leftLight = (1.0 - smoothstep(.0, .07, v_uv.x)) * .09;
      float bottomShade = (1.0 - smoothstep(.0, .09, v_uv.y)) * .08;
      color.rgb += edgeLens * .055 + topLight + leftLight - bottomShade;

      /* The expanded navigation uses the regular, thicker material: soften
         chroma and dynamic range before adapting density to the scene. */
      float sourceLuminance = dot(color.rgb, vec3(.2126, .7152, .0722));
      color.rgb = mix(vec3(sourceLuminance), color.rgb, mix(.9, .76, u_open));
      color.rgb = (color.rgb - .5) * mix(1.035, .9, u_open) + .5;
      ...
      float lightDensity = mix(.24, .52, u_open) + (1.0 - luminance) * .1;
      float darkDensity = mix(.28, .55, u_open) + luminance * .08;
